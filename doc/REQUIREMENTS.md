...
- **同一工作区多环境**：可为一个工作区添加多个环境连接（默认环境 `default` 与命名环境如 prod / dev / test），各环境拥有独立工具名与连接标识，互不混淆；可在设置卡片中直接查看、新增、修改、删除。
- **三种配置方式**：
  1. 直接填写完整连接串（DSN）；
...
- **变更即时生效**：卡片新增/修改/删除连接后，常驻 dbhub 服务立即重启并重新注册全部工具（含新环境的 `_<环境>` 工具），下一次模型请求即可直接调用，无需重启实例。
- **连接源枚举工具**：新增 `dbhub_list_sources` 工具，模型可随时列出全部已注册连接源（工作区 × 环境、打码连接串、来源与对应工具名），避免把「测试环境」误当成默认（生产）库。
- **脱敏连接串不可直连**：`dbhub_list_sources` 中展示的连接串密码为 `****`（仅识别用途）；`dbhub_query` / `dbhub_query_objects` 检测到脱敏密码时会直接拒绝并引导使用对应的常驻工具（内置真实凭据），避免用 `****` 白跑一次失败连接。
