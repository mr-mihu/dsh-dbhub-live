...
window.__ModuleLoader__.load({ id: "dsh-dbhub-live", factory: (require) => {
  ...
  function StatusCard(props) {
    ...
    var statusCells = [
      cell("server", "常驻进程", serverUp ? "运行中" : "待启动（按需）"),
      cell("mode", "工作模式", modeLabel),
      cell("tools", "工具声明", String(toolCount) + " 个（固定）"),
      cell("envs", "环境", String(workspaces.length) + " 个 · 已保存 " + String(savedCount)),
    ];
    if (lastError) {
      statusCells.push(cell("error", "最近错误", lastError, true));
    }
