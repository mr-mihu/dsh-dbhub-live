...
import {
  listWorkspaces, setWorkspaceEnv, removeWorkspaceEnv, renameWorkspaceEnv,
} from './config.mjs'
  ...
  try {
    if (op.op === 'add' || op.op === 'set') {
      ...
      if (renameFrom && renameFrom !== String(op.env == null ? '' : op.env).trim()) {
        ...
        if (moved) console.log('[dsh-dbhub-live] ' + currentT('log.opRenamed', { title: ws.title, from: renameFrom, to: moved }))
      }
      ...
      else console.log('[dsh-dbhub-live] ' + currentT('log.opRenamed', { title: ws.title, from: String(from || 'default'), to: renamed }))
