...
async function handleConfigOp(rawOp, ctx, subprocess) {
  ...
  try {
    ...
    console.warn('[dsh-dbhub-live] ' + currentT('log.syncLater', { msg: String((e && e.message) || e) }))
  }
  refreshToolCount()
}
  ...
  initInFlight = (async () => {
    try {
      ...
      if (isServerUp()) {
        ...
        state.clearError()
        state.setPhase('running')
        console.log('[dsh-dbhub-live] ' + currentT('log.readyNoSources'))
      } else {
        // A real start failure was already recorded into the state machine;
        // the next tool call retries automatically.
        console.error('[dsh-dbhub-live] ' + currentT('log.initFailed'))
      }
    } catch (e) {
