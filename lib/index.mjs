...
// ── settings namespace: live status mirror for the browser card ───────────
...
// never fail to boot over a status accessory.
...
// Mirrors the state machine into the settings namespace when a settings
// provider is mounted (the Plugins tab dispatches our card from this
// namespace). The namespace's `enabled` field is how the card's toggle writes
// back; the host store remains the authoritative source and converges the
// document at boot. Everything is wired on the injected settings fiber so it
// rolls back with this plugin.
function wireStatusNamespace(ctx) {
  ctx.inject(['settings'], (sctx) => {
    let scope
    try {
      scope = sctx.settings.register(STATUS_NS, STATUS_SCHEMA, { base: {} })
    } catch (e) {
      console.error('[dsh-dbhub-live] 状态命名空间注册失败: ' + String((e && e.message) || e))
      return
    }
    const publish = () => {
      scope.replace(state.snapshot()).catch((e) => {
        console.warn('[dsh-dbhub-live] 状态同步失败: ' + String((e && e.message) || e))
      })
    }
    const stopWatch = scope.watch((next) => {
      const nextEnabled = next && typeof next === 'object' && typeof next.enabled === 'boolean'
        ? next.enabled
        : undefined
      if (nextEnabled !== undefined && nextEnabled !== state.isEnabled()) {
