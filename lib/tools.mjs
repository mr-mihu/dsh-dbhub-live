...
function buildListSourcesTool(ctx, subprocess) {
  return {
    ...
    description:
      '列出当前已注册的全部数据库连接源（每个 = 工作区 × 环境）：工作区名、环境名、打码连接串、来源（已保存/自动发现）、工具名为 dbhub_execute_sql_<工作区>[_<环境>]。' +
    ...
    async execute() {
      ...
      const lines = rows.map((r) => {
        ...
        const slug = slugify(r.title) + '_' + (r.env === 'default' ? '' : '_' + envSlug(r.env))
        return '- ' + r.title + '（' + badge + '）｜环境 ' + r.env + '｜' + maskDsn(r.dsn) + '｜来源 ' + (r.source || '?') + '｜工具前缀 dbhub_*_' + slug
      })
      return { ok: true, text: '已注册连接源 ' + rows.length + ' 个：\n' + lines.join('\n') }
    },
  }
}
