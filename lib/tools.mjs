...
async function runSourceTool(ctx, subprocess, base, args, exec) {
  ...
  if (!src) {
    return {
      ok: false,
      text: '找不到数据源 "' + String((args && args.source) || '') + '"：请先调用 dbhub_list_sources 查看可用 source 值（如 <工作区> 或 <工作区>_<环境>）。',
    }
  }
  const mcpArgs = {}
  ...
  return runOnServer(ctx, subprocess, base + '_' + src.id, mcpArgs, exec, sourceLabel(src.row))
}
    ...
    async execute() {
      ...
      const lines = rows.map((r) => {
        ...
        return '- ' + r.title + '（' + badge + '）｜环境 ' + r.env +
          ...
          '｜source 值 ' + srcId + '（调用 dbhub_execute_sql / dbhub_search_objects 时填入）'
      })
      return {
        ok: true,
        text: '⚠️ 下面每个连接串都是脱敏显示（密码 ****），仅供识别，不要把它们拷进 dbhub_query 直连；查询某环境请用 dbhub_execute_sql / dbhub_search_objects，source 参数取该行的「source 值」。\n已注册连接源 ' + rows.length + ' 个：\n' + lines.join('\n'),
      }
    },
  }
}
